...
-- C2: CTE
WITH c2 AS (SELECT question.*,  COUNT(exam_id) AS exam_count
		FROM question
		LEFT JOIN exam_question USING (question_id)
		GROUP BY question_id)
SELECT *
FROM c2
WHERE exam_count = (SELECT MAX(exam_count)
                       FROM c2
                       );
...
SELECT question.content, COUNT(exam_id) AS exam_count
FROM exam_question
RIGHT JOIN question USING (question_id)
GROUP BY question_id; 

-- vd khác
SELECT question_id, question.content, COUNT(question_id) AS question_count
FROM question
LEFT JOIN answer USING(question_id)
GROUP BY question_id;
-- Question 8: Lấy ra Question có nhiều câu trả lời nhất
SELECT question.content, COUNT(*) AS so_cau_tra_loi
FROM answer
...
-- C1: SUBQUERY
SELECT question.content,  COUNT(*) AS question_count
FROM question
LEFT JOIN answer USING (question_id)
GROUP BY question_id
HAVING COUNT(*) =
...
-- C2: CTE
WITH c3 AS (SELECT question.*,  COUNT(question_id) AS question_count
		FROM question
		LEFT JOIN answer USING (question_id)
		GROUP BY question_id)
SELECT *
...
SELECT `group`.* , COUNT(account_id) AS so_account_trong_group 
FROM `group`
LEFT JOIN group_account USING(group_id)
GROUP BY group_id;

-- Question 10: Tìm chức vụ có ít người nhất
SELECT position.*, COUNT(*) AS chuc_vu_it_nguoi_nhat
FROM position
...
GROUP BY position_id
HAVING COUNT(*) =
	(SELECT MIN(chuc_vu_it_nguoi_nhat) 
	FROM 
		(SELECT   COUNT(*) AS chuc_vu_it_nguoi_nhat
		FROM position
		LEFT JOIN account USING (position_id)
		GROUP BY position_id) AS T);

-- C2: CTE
WITH c3 AS (SELECT position.*,  COUNT(position_id) AS chuc_vu_it_nguoi_nhat
		FROM position
		LEFT JOIN account USING (position_id)
		GROUP BY position_id)
SELECT *
FROM c3
WHERE chuc_vu_it_nguoi_nhat = (SELECT MIN(chuc_vu_it_nguoi_nhat)
                       FROM c3
                       );
...
SELECT question_id,type_question.type_name
FROM question
INNER JOIN type_question USING(type_id)
UNION
SELECT question_id, category_question.category_name
FROM question
INNER JOIN category_question USING(category_id);
-- Question 13: Lấy ra số lượng câu Hỏi của mỗi loại tự luận hay trắc nghiệm
SELECT type_question.type_name, COUNT(type_name) as soluongcauhoi
FROM type_question
...
SELECT type_question.type_name, COUNT(type_name) as soluongcauhoi
FROM type_question
INNER JOIN question USING(type_id)
WHERE type_id = 1
GROUP BY type_id
UNION
SELECT type_question.type_name, COUNT(type_name) as soluongcauhoi
...
SELECT group_id, COUNT(account_id) as sothanhviencuagroup
FROM `group`
