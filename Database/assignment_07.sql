...
BEGIN
	IF NEW.created_date < CURRENT_DATE - INTERVAL 1 YEAR THEN
		...
		SET MESSAGE_TEXT = "không cho người dùng tạo group trước 1 năm";
    END IF;
END $$
DELIMITER ;
...
BEGIN
	...
	IF NEW.department_id = v_sale_id  THEN
		SIGNAL SQLSTATE "12345"
		SET MESSAGE_TEXT = "Group không vượt quá 5 user";
    END IF;
END $$
DELIMITER ;
...
BEGIN
    ...
    SELECT COUNT(account_id) AS v_account_count
    FROM group_account
    WHERE group_id = NEW.group_id;

	IF v_account_count > 5  THEN
		SIGNAL SQLSTATE "12345"
		SET MESSAGE_TEXT = "Một group không được vượt quá 5 người";
    END IF;
END $$
DELIMITER ;

-- Question 4: Cấu hình 1 bài thi có nhiều nhất là 10 Question   exam_question
...
BEGIN
    ...
    SELECT COUNT(question_id) AS v_question_count
    FROM exam_question
    WHERE exam_id = NEW.exam_id;

	IF v_question_count > 10  THEN
		SIGNAL SQLSTATE "12345"
		SET MESSAGE_TEXT = "Một exam không được vượt quá 10 câu hỏi";
    END IF;
END $$
DELIMITER ;
...
SELECT  `group`.*,COUNT(account_id) as account_count,
     CASE
        ...
        ELSE "higher"
	END AS the_nnumber_user_amount
FROM `group`
